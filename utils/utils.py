...
def collate_MIL_survival_sig_hallmarks(batch):
    """Collate function for variable number of omic features (e.g., 50 Hallmark pathways)"""
    # batch[0] format: (wsi_features, omic1, omic2, ..., omicN, label, event_time, c, case_id)
    # Last 4 items are always label, event_time, c, case_id
    # For batch_size=1, just return the single item's tensors
    item = batch[0]
    n_items = len(item)

    img = item[0]  # WSI features [num_slides, 768]

    # Middle items are omics (from index 1 to n_items-4)
    # Keep them as 1D tensors (no unsqueeze) to match original format
    omics = [item[i].type(torch.FloatTensor) for i in range(1, n_items - 4)]

    label = torch.LongTensor([item[n_items - 4]])
    event_time = np.array([item[n_items - 3]])
    c = torch.FloatTensor([item[n_items - 2]])
    case_id = item[n_items - 1]  # String, keep as-is
...
def get_split_loader(split_dataset, training = False, testing = False, weighted = False, mode='coattn', batch_size=1):
    ...
    # num_workers=4 with pin_memory for faster data loading
