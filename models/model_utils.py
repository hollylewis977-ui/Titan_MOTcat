...
class FiLMGenerator(nn.Module):
    ...
    Uses sigmoid*2 to ensure gamma is in [0,2] range for numerical stability.

    Initialization Strategy: Last layer weights initialized to 0
    - Initial gamma = sigmoid(0) * 2 = 1.0
    - Initial beta = 0
    - Training starts in "no modulation" state, preventing early loss oscillation

    Args:
        context_dim (int): Dimension of context vector (default: 256)
        feature_dim (int): Dimension of features to modulate (default: 256)
        hidden_dim (int): Hidden layer dimension (default: 128)
        dropout (float): Dropout rate (default: 0.25)
    """
    def __init__(self, context_dim=256, feature_dim=256, hidden_dim=128, dropout=0.25):
        super().__init__()
        self.fc = nn.Sequential(
            nn.Linear(context_dim, hidden_dim),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(hidden_dim, feature_dim * 2)  # gamma + beta
        )
        self.feature_dim = feature_dim

        # Initialize last layer with small random values for stable training start
        # Initial output ≈ 0, so gamma ≈ sigmoid(0)*2 = 1.0, beta ≈ 0
        # But non-zero weights allow gradients to flow back through the network
    ...
    def forward(self, context):
        """
        Args:
            context: (N, context_dim) - context vector for each sample
        Returns:
            gamma: (N, feature_dim) - scaling factor in [0, 2]
            beta: (N, feature_dim) - bias term (unbounded)
        """
        out = self.fc(context)  # (N, feature_dim * 2)
        gamma = torch.sigmoid(out[:, :self.feature_dim]) * 2  # [0, 2] range
        beta = out[:, self.feature_dim:]  # unbounded
        return gamma, beta